...
#         basisset = '6-311ppg_d_p_'
        ...
        self.model = Psi4Model(method = 'scf', basis = basisset,
                  psi4_output = 'psi4_output.dat', cores=2, memory=3e+09)
    ...
    '''
    def write_centers(self, append=False, folder='xyz_files/', filename='None'):
        # Assign filename depending if it is given as an input
        # or if it has to be generated from the elements.
        if filename == 'None':
            self.filename = folder + self.generate_molname() + '.xyz'
        else:
    ...
    def write_orbitals(self, filename='None'):
        ...
        # or if it has to be generated from the elements.
        if filename == 'None':
            filename = 'molden-fchk_files/' + self.generate_molname()
        else:
...
#             grad = np.array(jax.jit(jax.grad(cost_function, argnums=0))(W))
            ...
            if i!=0:
                ...
                # In this case the resulting H is 'pointing' in the 
                # descent direction, so we reinitialize it.
...
#                     H = riemann_grad
    ...
    def perform_sweep(self, p_min=0., p_max=1., steps=10, scheme='V1'):
        ...
        filename = 'xyz_files/' + self.generate_molname()
